    ...
    return (
        <div className="fixed bottom-24 sm:bottom-6 right-4 z-50 flex flex-col gap-2 max-w-xs">
            {toasts.map((toast) => (
                <div
                    key={toast.id}
                    className={`rounded-lg px-4 py-3 text-sm font-semibold flex items-center gap-2 shadow-lg animate-in slide-in-from-bottom-2 duration-300 ${toast.type === 'success'
